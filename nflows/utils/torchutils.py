...
def searchsorted(bin_locations, inputs, eps=1e-3):
    bin_locations[..., -1] += eps

    return torch.searchsorted(bin_locations, inputs[..., None].detach(), right=True).squeeze() -1 #torch.sum(inputs[..., None] >= bin_locations, dim=-1) - 1
