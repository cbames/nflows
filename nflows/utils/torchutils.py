...
def searchsorted(bin_locations, inputs, eps=1e-6):
    ...
    return torch.sum(inputs[..., None] >= bin_locations, dim=-1) - 1#torch.searchsorted(bin_locations, inputs[..., None].detach(), right=True).squeeze() -1 #torch.sum(inputs[..., None] >= bin_locations, dim=-1) - 1
